...
statement : var_declaration
	  ...
	  | PRINTLN LPAREN ID RPAREN SEMICOLON
	  {
			...
			symbol_info * var_info = get_info($3->getname());
			if (var_info==NULL){
	  ...
	  | RETURN expression SEMICOLON
	  {
			...
			current_func_return_type = $2->get_data_type();
...
variable : ID 	
      {
		...
		outlog << $1->getname() << endl << endl;
		symbol_info* var_info = get_info($1->getname());
		...
		if (var_info==NULL){
			...
			$$->set_data_type(var_info->get_data_type()); // Set float flag if var_info is float
		}
...
expression : logic_expression
	   ...
	   | variable ASSIGNOP logic_expression 	
	   {
			...
			outlog << $1->getname() << "=" << $3->getname() << endl << endl;
				...
				}else if ($1->get_data_type()!=$3->get_data_type() && $3->get_is_undefined_function()==false){
...
simple_expression : term
          {
			...
			$$->set_data_type($1->get_data_type());  // Set data type of term
			if ($1->get_is_function()){
				$$->set_as_function($1->get_return_type(),$1->get_parameters());
			}
...
int main(int argc, char *argv[])
{
	...
	yyin = fopen(argv[1], "r");
	outlog.open("23241072_log.txt", ios::trunc);
	errlog.open("23241072_error.txt", ios::trunc);
