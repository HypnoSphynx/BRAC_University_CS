...
class VarNode : public ExprNode {
    private:
        ...
        string element_type; // Type of array elements (int, float, etc.)
    ...
    public:
        ...
        string generate_index_code(ofstream& outcode, map<string, string>& symbol_to_temp,
            ...
            if (!index) return "0"; //No index, return default
    ...
    public:
        ...
        string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
            ...
            if (lhs->has_index()) { //if array
                string array_temp = symbol_to_temp[lhs->get_name()]; //array base var fetch
                string idx_temp = lhs->generate_index_code(outcode, symbol_to_temp, temp_count, label_count);
                
                // Type check/conversion if needed
...
class DeclNode : public StmtNode {
    private:
        ...
        map<string, VarNode*> var_nodes; // Store VarNode references for array variables

    public:
        DeclNode(string t) : type(t) {}
        
        ~DeclNode() {
            // Clean up any VarNode pointers we own
        ...
        void add_var(string name, int array_size = 0) {
            ...
            // Create VarNode for array vars to track their element type
            if (array_size > 0) {
...
public:
    ...
    string generate_code(ofstream& outcode, map<string, string>& symbol_to_temp,
        ...
        //temp for the function call result
        string result_temp = "t" + to_string(temp_count++);
